...
# param: $3 = parametre de l'agresseur 3
    ...
    # echo "--> cd $AGGRESSOR3WORKINGSPACE"
    if [ ! -d $AGGRESSOR3WORKINGSPACE ]; then
	echo "*** Error: $AGGRESSOR3WORKINGSPACE not found"
	cd $BENCHMARKS
	exit 0
    fi
    cd $AGGRESSOR3WORKINGSPACE

    if [ ! -f aggressor_${AGGRESSOR3} ]; then
	echo "*** Error: aggressor_${AGGRESSOR3} not found"
	cd $BENCHMARKS
	exit 0
    fi
    cp aggressor_${AGGRESSOR3} ${AGGRESSOR3}_agresseur3
    # echo "--> exec: aggressor3 $3 &"
    taskset -c 3 ./${AGGRESSOR3}_agresseur3 $3 &
    pid_agresseur3=$!
